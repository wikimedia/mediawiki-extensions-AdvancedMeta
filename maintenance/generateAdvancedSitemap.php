...
class GenerateAdvancedSitemap extends Maintenance {
	...
	public function __construct() {
		...
		$this->addOption(
			'urlpath', 'The URL path corresponding to --fspath, prepended to filenames in the index; '
			. 'defaults to an empty string', false, true
		);
		$this->addOption(
			'compress', 'Compress the sitemap files, can take value yes|no, default yes', false, true
		);
	...
	public function execute() {
		...
		$this->identifier = $this->getOption( 'identifier', WikiMap::getCurrentWikiId() );
	...
	private function generateNamespaces() {
		...
		$res = $this->dbr->select( 'page', [ 'page_namespace' ], [], __METHOD__, [
	...
	public function main() {
		...
		foreach ( $this->namespaces as $namespace ) {
			...
			foreach ( $res as $row ) {
				...
				// skip if noindex is set with AdvancedMeta extension
				$title = Title::makeTitle( $row->page_namespace, $row->page_title );
				$metaData = $metaHandlerFactory->newFromTitle( $title )->getData();
...
$maintClass = GenerateAdvancedSitemap::class;
